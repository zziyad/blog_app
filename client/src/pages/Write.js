...
const Write = () => {
  ...
  const uploadPost = async (post) => {
    ...
    try {
      const pid = await method(post);
        console.log({ pid });
      navigate(`/post/${state ? parseInt(state.postid) : parseInt(pid.result)}`);
      return;
    } catch (error) {
  ...
  const handleSend = async (event) => {
    ...
    const post = {
      ...
      authorId: parseInt(currentUser?.accountId),
      file: "no file",
    };
    ...
    if (file) {
      readFile(file, async (err, res) => {
        ...
        try {
          ...
          post.file = call.result;
          return await uploadPost(post);
        } catch (error) {
          setError(error);
        }
      });
      ...
      <div className="menu">
        <div className="item">
          ...
          <div className="buttons">
            ...
            <button className="blc" onClick={(e) => handleSend(e)}>
              Update
            </button>
          </div>
        </div>
